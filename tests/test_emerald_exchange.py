...
def test_diamond_command_and_result_preserve_uuid_count_and_request_mapping() -> None:
    command = diamond_emerald_exchange_command(PLAYER_UUID, 4, REQUEST_ID)
    result = parse_diamond_emerald_exchange_result(
        f"USAPO_DIAMOND_EXCHANGE_RESULT|1|{REQUEST_ID}|completed|4|64|new",
        expected_request_id=REQUEST_ID,
        expected_diamond_count=4,
    )

    assert command == (f"usapo-event-bridge diamond-emerald-v1 {PLAYER_UUID} 4 {REQUEST_ID}")
    assert result.status == "completed"
    assert result.diamond_count == 4
    assert result.emerald_count == 64
    assert not result.duplicate

    with pytest.raises(ValueError, match="1 or 4"):
        diamond_emerald_exchange_command(PLAYER_UUID, 2, REQUEST_ID)
...
def test_parses_diamond_exchange_audit_event() -> None:
    event = parse_diamond_emerald_exchange_event(
        "[12:34:56] [Server thread/INFO]: [UsapoEventBridge] "
        f"USAPO_DIAMOND_EXCHANGE|1|{REQUEST_ID}|{PLAYER_UUID}"
        "|Lll1a2kxOTkx|4|64|1786406400000"
    )
...
def test_bot_executes_atomic_diamond_plugin_command_for_linked_uuid(tmp_path) -> None:
    bot = MinecraftDiscordBot(Config(discord_token="test", accounts_path=tmp_path / "db"))
    account = SimpleNamespace(player_uuid=PLAYER_UUID)
    bot._online_exchange_account = AsyncMock(  # type: ignore[method-assign]
        return_value=(account, None)
    ...
    bot._execute_rcon = AsyncMock(  # type: ignore[method-assign]
        return_value=(f"USAPO_DIAMOND_EXCHANGE_RESULT|1|{REQUEST_ID}|completed|1|16|new")
    )
    interaction = SimpleNamespace(user=SimpleNamespace(id=123))

    result = asyncio.run(
        bot.confirm_diamond_emerald_exchange(  # type: ignore[arg-type]
            interaction,
            request_id=REQUEST_ID,
            diamond_count=1,
        )
    ...
    bot._online_exchange_account.assert_awaited_once_with(123)  # type: ignore[attr-defined]
    bot._execute_rcon.assert_awaited_once_with(  # type: ignore[attr-defined]
        f"usapo-event-bridge diamond-emerald-v1 {PLAYER_UUID} 1 {REQUEST_ID}"
    )
...
def test_structured_diamond_exchange_log_wires_uuid_to_discord_audit(tmp_path) -> None:
    class OneLineTailer:
        def __init__(self, line: PendingLine) -> None:
            self.line = line
            self.acknowledged: list[PendingLine] = []
    ...
    line = PendingLine(
        text=(
            "[12:34:56] [Server thread/INFO]: [UsapoEventBridge] "
            f"USAPO_DIAMOND_EXCHANGE|1|{REQUEST_ID}|{PLAYER_UUID}"
            "|U3RldmU|1|16|1786406400000"
        ),
        cursor=Cursor("log-1", 124),
    )
    tailer = OneLineTailer(line)
    bot._tailer = tailer  # type: ignore[assignment]
    bot.wait_until_ready = AsyncMock()  # type: ignore[method-assign]
