...
def test_runtime_player_name_is_refreshed_by_uuid_from_usercache(tmp_path) -> None:
    async def exercise() -> None:
        ...
        bot._accounts.update_player_profile = Mock(  # type: ignore[method-assign]
            side_effect=AssertionError("unchanged profile must not be written")
        )
        matched_again = await bot._find_account_for_player_name(".yuki19911261")
