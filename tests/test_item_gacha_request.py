...
def test_parses_uuid_based_category_game_command_request() -> None:
    line = (
        "[08:24:00] [Server thread/INFO]: [UsapoEventBridge] "
        "USAPO_ITEM_GACHA_REQUEST|3|11111111-1111-4111-8111-111111111111|"
        "22222222-2222-4222-8222-222222222222|Lll1a2kxOTkx|equipment|premium|"
        "1000|1786838400000"
    )
...
def test_parses_legacy_request_without_trusting_an_unconfirmed_price() -> None:
    ...
    assert request.draw_category == "all"
    assert request.expected_cost_xp is None


def test_version_two_request_maps_to_all_category() -> None:
    line = (
        "[08:24:00] [Server thread/INFO]: [UsapoEventBridge] "
        "USAPO_ITEM_GACHA_REQUEST|2|11111111-1111-4111-8111-111111111111|"
        "22222222-2222-4222-8222-222222222222|U3RldmU|normal|100|1786838400000"
    )
...
def test_rejects_unknown_version_three_category() -> None:
    line = (
        "[08:24:00] [Server thread/INFO]: [UsapoEventBridge] "
        "USAPO_ITEM_GACHA_REQUEST|3|11111111-1111-4111-8111-111111111111|"
        "22222222-2222-4222-8222-222222222222|U3RldmU|food|normal|100|"
        "1786838400000"
    )
