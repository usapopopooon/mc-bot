...
伐採対象の原木・幹を設置すると、そのプレイヤーの連続伐採コンボを即座にリセットし、
同じ原木を置いて壊す反復ではコンボを継続できません。
...
`world_2`（内部名 `resource`）と、その専用ネザー `world_2_nether`・専用エンド
`world_2_the_end` では、ガチャ・フリマ・交換・クエストを利用できません。
ゲーム内コマンド／画面はPaper側、Discordの各パネル／操作はmc-bot側で、
利用できない旨と `world_1` 側へ戻る案内を本人だけに表示します。
Discordで既に開いていた確認画面や処理待ちのゲーム内要求も、現在のワールドを再確認します。

現在位置の判定は、Paperの読み取り専用RCON `economy-access` にUUIDで照会します。
応答不明・旧プラグインでは新しい取引を進めません。ガチャ・XP交換・資源交換の配布は、
サーバー内の同一コマンドでUUIDと3ワールドの除外条件を再確認するため、
決済待ち中にワールドを移動しても制限先には配布しません。
明確な配布拒否では既存の予約取消処理を使い、応答不明は二重配布防止の監査待ちを維持します。
完了済み取引の照合・通知、退会整理、Bot発行クエストの管理操作は継続します。
オフライン時の既存の非インベントリ操作、通常XP・進捗・釣り・伐採・VC報酬は変更しません。

Minecraft 資源交換所では、既存のサーバーXPから資源への交換に加えて、オンライン中の
連携アカウントがエメラルド32/64個をダイヤモンド1/2個へ、ダイヤモンド1/4個を
エメラルド16/64個へ両替できます。交換パネルの「交換内容」にはサーバーXPから資源への交換と
...
サーバーXP→資源、ダイヤ・エメラルドの両替、手持ち資源→サーバーXP、XP残高確認を
利用できます。資源売却はエメラルド64個を500 XPで、土・砂・砂岩・深層岩・
深層岩の丸石・凝灰岩を64個単位で所定レートにより扱い、
1人1日3,000サーバーXP（毎日0時・日本時間に更新）までです。名前や特殊データ付きの
アイテムは回収しません。売却完了時は、現在のサーバーXPと当日の残り売却枠も表示します。
スマホ版を含むFloodgate/Bedrockでは選択フォーム、Java版ではチェスト画面と確認画面を
表示します。画面を表示できない場合は `/exchange xp <50|250|500|5000>`、
`/exchange resource <資源ID> <個数>`（現在の商品はTab補完）、
`/exchange emerald-diamond <32|64>`、`/exchange diamond-emerald <1|4>`、
対象アイテムをメインハンドに持って `/exchange buyback <1|2|4|8|16|max|all>`、
...
資源売却APIが入力を確定的に拒否した場合は、Minecraft側の保留を解除してアイテムを
回収せず、該当ログを完了扱いにして後続の出品・購入・ガチャ処理を継続します。
通信断やサーバーエラーのように成否が不明な場合だけ、同じ要求IDで再試行します。
処理待ち中にMinecraftログがローテーションしても、開いている旧ログの未処理行を
読み切ってから新しいログへ切り替えます。

資源交換の商品・価格は、サーバー管理権限を持つユーザーがDiscordから
`/mc-config resource-exchange-set`、`resource-exchange-remove`、
`resource-exchange-list` で管理できます。level-botへ世代付きの全カタログを保存した後、
mc-botがRCONでPaperプラグインへ即時同期するため、導入後の日常的な商品・価格変更では
Minecraft再起動は不要です。RCON停止中でも設定は失われず、30秒周期の処理で再同期します。
Paper側も最後に検証・保存できたカタログを保持し、壊れたデータや古い世代へは切り替えません。

Minecraft アイテムガチャは、オンライン中の連携プレイヤーがDiscordの常設パネル、または
ゲーム内の `/gacha` から利用できます。最初に「おまかせ」「資源・採掘」「冒険」
「装備・強化」を選び、その後に通常100 XPまたはR以上確定1,000 XPを選びます。
スマホ版を含むFloodgate/BedrockとDiscordでは二段階の選択画面を表示します。Java版と
フォームを表示できない場合は、従来どおり `/gacha normal` または `/gacha rare` で
おまかせを引けます。種類を指定する場合は、たとえば `/gacha resource normal`、
`/gacha adventure rare`、`/gacha equipment normal` を使います。すべての種類と引き方を
合わせて日本時間0:00区切りで合計1日3回までです。Discordの確認画面では現在XPと
抽選後のXPを表示し、ゲーム内の処理結果は実行者だけに表示します。公開するランク確率は、通常が
N 47%、R 25%、SR 14.5%、SSR 7%、UR 4%、幻 2.5%、R以上確定が
R 20%、SR 40%、SSR 23%、UR 12%、幻 5%です。景品内容と個別確率は抽選・受取が
完了するまで公開しません。抽選結果をSQLiteへ先に固定してから、固定許可リストの
景品だけをRCONで付与します。カテゴリはランク抽選後の景品候補だけを絞り、公開ランク確率、
料金、1日上限は変えません。用途が重なる景品は複数カテゴリに含まれます。
同日連打やBot再起動でカテゴリや景品を変えず、RCON応答を失って付与結果を
断定できない場合も二重配布を避けるため自動再送しません。Nを含む全結果をMinecraft全体
チャットとDiscordログへ投稿し、エンチャント済み装備は実際の効果とレベルも併記します。
Discordで通知を許可するのは抽選者本人だけで、
...
予約します。料金を含まない旧Paper形式、または料金改定後の古い画面から届いた要求では
XPを消費しません。
...
自分の出品、返却受取箱、所持XP確認をフォームで操作でき、Java版では実際のアイテムを並べたチェスト画面と
数字ボタンで同じ操作ができます。画面を表示できない場合も `/market list [ページ]`、
`/market sell <合計価格XP>`、`/market buy <出品番号>`、`/market mine`、
`/market cancel <出品番号>`、`/market claim`、`/market balance` を利用できます。Discordでは設定したチャンネルへ
商品カードと短い説明パネルを投稿します。説明パネルは新しい投稿の後にチャンネル末尾へ戻り、
「使い方」で詳しい操作方法、「サーバーXP確認」で所持サーバーXPを本人だけ確認できます。
...
カスタムアイテムはその名前を保持します。トライデントを含む通常装備は全エンチャント名と
レベルを商品名へ併記します。エンチャント本は保存されているエンチャント名と
レベルも商品名へ併記し、5種類以上の場合は先頭4種類と残りの種類数を表示します。
購入確認には現在XPと購入後XPを表示します。受取成功後に表示価格を買い手から差し引いて
売り手へ加算します。Discordからの購入、出品取消、アイテム返却には
対象のMinecraftアカウントがオンラインで、インベントリに空きがある必要があります。
取引中はXPを予約し、明確な配布失敗では予約を解放します。応答が不明な場合は自動で二重配布
せず、同じ取引IDで再確認できる状態を保持します。
管理対象の参加登録を解除した時やDiscordサーバーから退会した時は、売れていない出品を
自動で取り消し、商品をMinecraft UUID単位の永続「フリマ返却受取箱」へ移します。
購入予約中の出品は先に購入結果を確定し、不成立で出品へ戻った場合だけ返却受取箱へ移します。
再登録して戻った後、`/market` の「返却受取箱」または `/market claim` から受け取れます。
退会整理の応答が不明な場合は登録を `pending_remove` に保ち、同じ操作IDで再試行します。
Whitelist解除は取引整理より先に行うため、整理の一時失敗で参加権限だけが残ることはありません。

ギルド・クエスト掲示板では、連携済みプレイヤーが通常アイテムまたはエンチャント本の
一括納品を依頼できます。
Java版・Floodgate/Bedrock版とも、Minecraftで依頼品を手に持って `/quest` を開き、画面から
個数と期限を選んだあと、報酬スタックを持って公開内容を確認します。画面を表示できない場合は
`/quest create <個数> <期限時間>` と `/quest confirm` も利用できます。通常アイテムは名前・
エンチャント等のないスタック可能アイテムに限り、エンチャント本は依頼品・報酬のどちらにも
使用できます。依頼された本は保存エンチャントの種類・レベル・表示名が一致する場合だけ
納品でき、画面には保存エンチャントを省略せず表示します。見えない金床の使用履歴は判定に
含めません。エンチャント本の依頼数は1冊固定で、Java版・Bedrock版とも個数入力を省略します。
依頼品・報酬とも1スタック以内、受注後の期限は1〜72時間です。
報酬は公開前にPaper側へ預けられ、受注者は1人だけです。受注後は依頼者から取り消せず、
受注者の辞退または期限切れでは再募集されます。未受注のまま7日経過すると終了し、報酬を
返却します。報酬、返却品、納品物は永続受取箱へ入り、Minecraftの `/quest` にある受取操作、
または画面を表示できない場合の `/quest claim` で、インベントリに空きがある時だけ
重複なく受け取れます。
管理対象の参加登録を解除した時やDiscordサーバーから退会した時は、そのプレイヤーが
依頼者である未完了クエストを無効化して報酬を受取箱へ戻し、受注者であるクエストは
受注解除して再募集します。いずれもオンライン状態には依存せず、途中失敗は再試行されます。

Discordの `/mc-config quest-channel` で掲示板を設置すると、募集中の依頼だけをカード表示し、
受注・使い方・自分のクエスト・受取方法をボタンから操作できます。各公開カードには
「受注」と「依頼取消」があり、通常の依頼取消は募集中の依頼主本人だけが実行できます。
Minecraft管理メニューの「Bot発行クエスト」では、「サーバーの管理」権限を持つ管理者が
依頼品・報酬・個数・受注後の期限を入力し、日本語名・英語名・IDの検索候補から実際のアイテムを選んで
最終確認後に公開できます。完全一致しない名前は自動決定せず、候補選択が済むまで公開できません。
Bot発行分はMinecraftインベントリや連携アカウントを必要とせず、依頼者をDiscordのBotメンション、
Minecraft名を `-` として表示します。報酬は達成時だけ受注者の受取箱へ生成し、納品物は回収します。
管理者が募集中のBot発行分を取り消した場合や募集期限切れの場合、架空の報酬返却・通知は生成しません。
公開カードの受注・依頼取消と、本人専用画面の取消・納品・辞退は、内容を再表示する
確認画面を経て確定します。自分のクエストは
前後ボタンで全件を確認でき、個人画面の「戻る」でクエストメニューへ戻れます。
再デプロイ・Bot再起動時は保存済みの説明パネルをその場で
更新し、削除されている場合だけ再作成します。保存済みの募集カードに必要な操作ボタンが
不足している場合だけ、そのカードを更新します。受注済みカードは
...
構文不正・未対応バージョン・既存IDと内容が矛盾する構造化イベントは、その1件だけを
隔離してカーソルを進め、後続イベントの処理を継続します。設定済みのMinecraftログ通知先には
イベント種別・理由・ログ位置を含むエラーを投稿します。エラー投稿自体に失敗しても後続処理は
止めません。通信障害やRCON応答喪失など処理結果が不明な場合は、不正イベントとは扱わず、
二重処理を避けるため従来どおり同じIDで再試行します。
...
/mc-config resource-exchange-set item_id:copper_ingot item_name:銅インゴット item_count:16 cost_xp:240
/mc-config resource-exchange-list
...
現在のWhitelist全件の一覧表示、Bot発行クエストの作成ができます。Discord側を修正する場合はWhitelistを維持し、
